...
export function getLocalBirthdays(): BirthdayData[] {
  try {
    ...
    return Array.isArray(parsed)
      ? parsed.map((item) => normalizeBirthdayRecord(item))
      : [SAMPLE_BIRTHDAY];
  } catch (err) {
...
export function saveLocalBirthdays(birthdays: BirthdayData[]) {
  try {
    ...
    console.warn(
      'LocalStorage quota exceeded, attempting pruned save',
      err
    );
    ...
    try {
      const pruned = birthdays.map((b) => ({
        ...b,
        profile_image_url: isDataUrl(b.profile_image_url)
          ? ''
          : b.profile_image_url,
        memory_image_urls: b.memory_image_urls.map((img) =>
          isDataUrl(img) ? '' : img
        ),
        music_url: isDataUrl(b.music_url || '')
          ? null
          : b.music_url,
      }));
      ...
      console.error(
        'Failed to save to local storage even after pruning',
        e
      );
    }
  }
...
export async function uploadMediaFile(
  ...
  if (isSupabaseConfigured && supabase) {
    try {
      ...
      if (uploadError) {
        console.warn(
          `Supabase storage upload error for ${targetBucket}, using fallback:`,
          uploadError
        );
      } else if (uploadData) {
...
export function encodeBirthdayToUrlPayload(
  ...
  try {
    ...
    const compactObj = {
      ...
      p: isDataUrl(data.profile_image_url)
        ? ''
        : data.profile_image_url,
      m: data.memory_image_urls.filter(
        (url) => !isDataUrl(url)
      ),
    };
...
export function decodeBirthdayFromUrlPayload(
  ...
  try {
    ...
    console.error(
      'Failed to decode birthday payload from URL',
      e
    );
 ...
 *
 * Every QR/share URL uses this domain.
 */
 ...
 *
 * Environment variables and browser origin cannot replace it.
 */
...
/**
 * Generate the unique public birthday URL.
 *
 * Example:
 * https://birthday-surprise-vert-rho.vercel.app/birthday/ABC123
 */
export function getBirthdayShareUrl(
  birthday: BirthdayData
): string {
  return `${PRODUCTION_ORIGIN}/birthday/${birthday.slug}`;
}
...
> {
  ...
  if (isSupabaseConfigured && supabase) {
    try {
      ...
      if (!error && data) {
        supabaseBirthdays = data.map(
          (item) => normalizeBirthdayRecord(item)
        );
      }
    } catch (err) {
      console.warn(
  ...
  [...localBirthdays, ...supabaseBirthdays].forEach(
    (item) => {
...
export async function createBirthday(
  ...
  const recordToInsert = {
    ...
    memory_image_urls:
      ...
      input.memory_image_urls.length > 0
        ? input.memory_image_urls
        : SAMPLE_BIRTHDAY.memory_image_urls,
    intro_text:
      input.intro_text ||
    ...
    theme_id:
      input.theme_id || 'sakura-dream',
    relationship_type: rel,
    experience_type: rel,
    design_id:
      input.design_id || 'romantic-rose',
    relationship_role:
      input.relationship_role || null,
    nickname: input.nickname || null,
    custom_ending_message:
      input.custom_ending_message || null,
    story_data:
      input.story_data ||
      SAMPLE_BIRTHDAY.story_data,
    created_at: now,
