import React, { useState } from 'react';
import { Zap, Share2, RotateCcw, PartyPopper } from 'lucide-react';
import confetti from 'canvas-confetti';
import type { BirthdayData, ThemeConfig } from '../../../types/birthday';
import { LiveCountdown } from '../LiveCountdown';
import { MemoryGallery } from '../MemoryGallery';
import { StoryTimelineView } from '../StoryTimelineView';
import { BirthdayLetterView } from '../BirthdayLetterView';
import { WishCardsGrid } from '../WishCardsGrid';
import { getExperienceConfig } from '../../../config/experienceConfig';

interface Props {
  birthday: BirthdayData;
  theme: ThemeConfig;
  onReplayIntro?: () => void;
}

export const BestFriendExperience: React.FC<Props> = ({
  birthday,
  theme,
  onReplayIntro,
}) => {
  const config = getExperienceConfig('bestFriend');
  const [finaleRevealed, setFinaleRevealed] = useState(false);

  const triggerEmojiBlast = () => {
    setFinaleRevealed(true);

    // Regular confetti
    confetti({
      particleCount: 120,
      spread: 90,
      origin: { y: 0.6 },
      colors: ['#ff007f', '#00f0ff', '#ffe600', '#a100ff', '#ff5400'],
    });

    // Fire shape confetti
    try {
      const scalar = 2;
      const emojiParticle = confetti.shapeFromText({ text: '🔥', scalar });
      const clownParticle = confetti.shapeFromText({ text: '😂', scalar });
      const partyParticle = confetti.shapeFromText({ text: '🎉', scalar });

      confetti({
        shapes: [emojiParticle, clownParticle, partyParticle],
        scalar,
        particleCount: 40,
        spread: 80,
        origin: { y: 0.6 },
      });
    } catch {
      // Fallback standard confetti
    }
  };

  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: `Happy Birthday to the most annoying bestie, ${birthday.name}! 😂🎉`,
        text: `Check out this crazy birthday surprise for ${birthday.name}!`,
        url: window.location.href,
      }).catch(() => {});
    } else {
      navigator.clipboard.writeText(window.location.href);
      alert('Bestie surprise link copied to clipboard! 📋🔥');
    }
  };

  return (
    <div className="space-y-16 pb-24 max-w-4xl mx-auto px-4">
      {/* 1. PLAYFUL BEST FRIEND HERO SECTION */}
      <section className="relative pt-10 pb-8 text-center overflow-hidden">
        {/* Warning sticker banner */}
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-400 text-slate-950 font-black text-xs uppercase tracking-wider shadow-lg mb-6 animate-pulse">
          <span>🚨 WARNING: HIGH CRINGE & CHAOS AHEAD 🚨</span>
        </div>

        <div className="max-w-2xl mx-auto space-y-6 relative z-10">
          {/* Polaroid style Avatar with taped border */}
          <div className="relative inline-block">
            <div className="absolute -top-6 -right-6 z-20 rotate-12">
              <span className="px-3 py-1 bg-pink-500 text-white font-black text-xs rounded-lg shadow-md uppercase">
                Bestie Alert! 🔥
              </span>
            </div>

            <div
              className="p-3 bg-white rounded-3xl shadow-2xl rotate-[-2deg] hover:rotate-0 transition-transform duration-300 border-4 border-slate-900 cursor-pointer group"
              onClick={triggerEmojiBlast}
              title="Tap for emoji chaos! 😂"
            >
              <div className="w-48 h-48 sm:w-56 sm:h-56 rounded-2xl overflow-hidden bg-slate-100">
                <img
                  src={birthday.profile_image_url}
                  alt={birthday.name}
                  className="w-full h-full object-cover group-hover:scale-105 transition-transform"
                />
              </div>
              <div className="pt-2 text-center">
                <span className="font-mono font-bold text-xs text-slate-500">
                  {birthday.nickname || 'Certified Trouble Maker'} #001
                </span>
              </div>
            </div>
          </div>

          {/* Headline */}
          <div className="space-y-3">
            <h1 className="text-4xl sm:text-6xl font-black tracking-tight text-slate-900">
              Happy Birthday,{' '}
              <span className="bg-clip-text text-transparent bg-gradient-to-r from-pink-500 via-purple-600 to-cyan-500">
                {birthday.name}
              </span>{' '}
              😂🎉
            </h1>

            <p className="text-base sm:text-xl font-bold text-slate-700 max-w-md mx-auto leading-relaxed">
              "Today we celebrate the birth of my absolute favorite pain in the neck!"
            </p>
          </div>
        </div>
      </section>

      {/* 2. COUNTDOWN */}
      <LiveCountdown targetDate={birthday.birthday_date} theme={theme} />

      {/* 3. EXCLUSIVE: FRIENDSHIP STATS COUNTER GRID */}
      <section className="space-y-6">
        <div className="text-center space-y-2">
          <span className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider bg-slate-900 text-white shadow-md">
            <Zap className="w-3.5 h-3.5 text-amber-400 fill-amber-400" />
            {config.exclusiveFeatures.featureTitle}
          </span>
          <h2 className="text-3xl sm:text-4xl font-black text-slate-900">
            Our Official Friendship Metrics 📊
          </h2>
          <p className="text-sm font-bold text-slate-500">
            {config.exclusiveFeatures.featureSubtitle}
          </p>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3.5">
          {config.exclusiveFeatures.items?.map((item) => (
            <div
              key={item.id}
              className="p-4 sm:p-5 rounded-3xl bg-white border-2 border-slate-900 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_rgba(15,23,42,1)] transition-all space-y-1.5 text-center"
            >
              <span className="text-3xl sm:text-4xl block">{item.icon}</span>
              <span className="text-xl sm:text-2xl font-black text-pink-600 block">
                {item.score}
              </span>
              <h3 className="text-xs sm:text-sm font-black text-slate-800 uppercase tracking-tight">
                {item.title}
              </h3>
              <p className="text-[11px] font-semibold text-slate-500 line-clamp-1">
                {item.description}
              </p>
            </div>
          ))}
        </div>
      </section>

      {/* 4. MEMORIES GALLERY */}
      {birthday.memory_image_urls?.length > 0 && (
        <MemoryGallery images={birthday.memory_image_urls} theme={theme} />
      )}

      {/* 5. FRIENDSHIP CHRONICLES TIMELINE */}
      {birthday.story_data?.length > 0 && (
        <StoryTimelineView stories={birthday.story_data} theme={theme} />
      )}

      {/* 6. BESTIE LETTER / ROAST */}
      {birthday.birthday_message && (
        <BirthdayLetterView
          message={birthday.birthday_message}
          senderName={birthday.sender_name}
          theme={theme}
        />
      )}

      {/* 7. WISH CARDS */}
      <WishCardsGrid theme={theme} />

      {/* 8. CRAZY FINALE CELEBRATION */}
      <section className="text-center space-y-6 pt-6">
        {!finaleRevealed ? (
          <div className="p-8 sm:p-12 rounded-3xl bg-white border-4 border-slate-900 shadow-[8px_8px_0px_0px_rgba(15,23,42,1)] space-y-6 max-w-xl mx-auto">
            <div className="w-20 h-20 rounded-2xl mx-auto bg-pink-500 text-white flex items-center justify-center border-2 border-slate-900 shadow-md animate-bounce text-4xl">
              💥
            </div>

            <div className="space-y-2">
              <h3 className="text-2xl sm:text-3xl font-black text-slate-900">
                Ready For The Grand Bestie Blast? 💣
              </h3>
              <p className="text-sm font-bold text-slate-600">
                Detonate the ultimate meme explosion!
              </p>
            </div>

            <button
              type="button"
              onClick={triggerEmojiBlast}
              className="px-8 py-4 rounded-2xl bg-gradient-to-r from-pink-500 via-purple-600 to-cyan-500 text-white font-black text-lg border-2 border-slate-900 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_rgba(15,23,42,1)] transition-all cursor-pointer inline-flex items-center gap-2"
            >
              <PartyPopper className="w-5 h-5" />
              <span>Launch Emoji Blast! 🎉😂</span>
            </button>
          </div>
        ) : (
          <div className="p-8 sm:p-12 rounded-3xl bg-white border-4 border-slate-900 shadow-[8px_8px_0px_0px_rgba(15,23,42,1)] space-y-6 max-w-xl mx-auto animate-in zoom-in-95 duration-500">
            <div className="w-20 h-20 rounded-2xl mx-auto bg-amber-400 text-slate-900 flex items-center justify-center border-2 border-slate-900 text-4xl animate-spin">
              🫂
            </div>

            <div className="space-y-3">
              <h3 className="text-3xl sm:text-4xl font-black text-slate-900">
                Happy Birthday, Bro/Sis! 🫂🔥
              </h3>
              <p className="text-base sm:text-lg font-bold text-slate-700 leading-relaxed max-w-md mx-auto whitespace-pre-line">
                "{birthday.custom_ending_message || config.defaultEndingMessage}"
              </p>
            </div>

            <div className="pt-2">
              <span className="block text-xs font-bold uppercase tracking-wider text-slate-400">Partner In Crime,</span>
              <span className="text-2xl font-black text-pink-600">
                {birthday.sender_name} 💥
              </span>
            </div>

            <div className="flex flex-wrap items-center justify-center gap-3 pt-3">
              <button
                type="button"
                onClick={triggerEmojiBlast}
                className="px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider bg-slate-900 text-white border border-slate-800 shadow-md hover:bg-slate-800 transition-transform hover:scale-105 cursor-pointer"
              >
                🔥 More Fire & Chaos!
              </button>
              <button
                type="button"
                onClick={handleShare}
                className="flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-xs font-bold bg-pink-50 text-pink-700 border border-pink-200 shadow-sm hover:bg-pink-100 transition-transform hover:scale-105 cursor-pointer"
              >
                <Share2 className="w-3.5 h-3.5 text-pink-600" /> Share Roast Link
              </button>
            </div>
          </div>
        )}

        {onReplayIntro && (
          <div className="pt-4">
            <button
              type="button"
              onClick={onReplayIntro}
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-black uppercase tracking-wider bg-white text-slate-800 border-2 border-slate-900 shadow-[3px_3px_0px_0px_rgba(15,23,42,1)] hover:translate-x-[1px] hover:translate-y-[1px] transition-all cursor-pointer"
            >
              <RotateCcw className="w-3.5 h-3.5" /> Replay Warning Intro 🚨
            </button>
          </div>
        )}
      </section>
    </div>
  );
};
